...
@foreach($beforeAfters as $beforeAfter)
          ...
          <div class="cbp-item text-center bodycontouring dark-wrapper inverse-text shadow rounded">
            <figure class="overlay overlay4 rounded">
			  ...
			  <div class="meta mb-3 text-capitalize"><span class="date">April 12, 2019</span> <span class="author">{{$beforeAfter->user()->company}}</span> <span><i class="fa fa-thumbs-up"></i> 10</span> <span class="category">{{$beforeAfter->getCategories()[0]->name}}</span>
					</div>
			  </div>
          </div>
@endforeach
